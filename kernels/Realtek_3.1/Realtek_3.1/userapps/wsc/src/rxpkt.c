...
void	record_and_check_AuthFail_v2(CTX_Tp pCtx , unsigned char* attackAddr)
{
	int i;
	struct sysinfo info ;
	unsigned int time_offset ;	
	time_t AutoLockDown_attack_time;	
	int LockDownTimeTmp=0;
	...
	/* get current time */ 
	sysinfo(&info);			
	AutoLockDown_attack_time = (int)info.uptime;


	if(pCtx->ADL_pin_attack_count==0){
		pCtx->ALD_recount_start_time = (int)info.uptime;
		RX_DEBUG("	init previous attack time =%d\n",(int)pCtx->ALD_recount_start_time);		
	}
	...
	time_offset = 
		difftime( AutoLockDown_attack_time , pCtx->ALD_recount_start_time);

	RX_DEBUG("	ADL_pin_attack_count=[%d]\n",pCtx->ADL_pin_attack_count);
	RX_DEBUG("	This attack time=%d\n",(int)AutoLockDown_attack_time);
	RX_DEBUG("	Previous attack time=%d ,time_offset=%d \n",(int)pCtx->ALD_recount_start_time ,time_offset);


	/*if current attack time between previous attack attack > ALD_RESET_TIME (now it's 1 hour)*/
	if(time_offset > ALD_RESET_TIME){
		pCtx->ADL_pin_attack_count=1;
		RX_DEBUG("	After (ALD_RESET_TIME[%d]) reset linear increase\n",ALD_RESET_TIME);
	}

	/*after compare , keep this attack time as previous , use for compare when next time attack  */ 
	pCtx->ALD_recount_start_time = AutoLockDown_attack_time ;  
	WSC_DEBUG("	Update previous attack time=%d \n",(int)pCtx->ALD_recount_start_time );			

	if(pCtx->ADL_pin_attack_count<=ALD_START_INCRESE){
			...
			( (pCtx->ald_t - pCtx->ald_h) == ( AUTH_FAIL_TIMES - 1)) )
		{
			...
			if(time_offset < AUTH_FAIL_TIME_TH){
				...
				sprintf(tmpbuf, "echo \"StartFrom[uptime]:%d,locktime:%d,attacker:%02X%02X%02X:%02X%02X%02X\" > %s" ,
					(int)AutoLockDown_attack_time, pCtx->auto_lock_down ,
					attackAddr[0],attackAddr[1],attackAddr[2],attackAddr[3],attackAddr[4],attackAddr[5],WSCD_LOCK_STAT);
		...
		LockDownTimeTmp = (pCtx->ADL_pin_attack_count-ALD_START_INCRESE) + 1 ;
		RX_DEBUG("	AutoLockDown_timeout=[%d]minutes\n\n" , (LockDownTimeTmp*LockDownTimeTmp) );	

		pCtx->auto_lock_down = (LockDownTimeTmp*LockDownTimeTmp)*ALD_UNIT_TIME ; // unit seconds
...
#ifdef WPS2DOTX		
		if(pCtx->is_ap==1 && pCtx->assigned_auth_type == AUTH_WPAPSK){
...
#ifdef WPS2DOTX		
		if(pCtx->is_ap==1 && pCtx->assigned_encrypt_type == ENCRYPT_TKIP){
...
static int decrypt_setting(CTX_Tp pCtx, STA_CTX_Tp pSta, unsigned char *pMsg, int msg_len)
{
	...
	while(1){		
		...
		if(PreCred==NULL){
			...
			RX_DEBUG("PreCredSize=%02x \n",PreCredSize);	
			debug_out("This run Credent:",PreCred , PreCredSize);			
...
#ifdef DEBUG
	if(pCtx->debug2){
		debug_out("M3 E-Hash1", pSta->e_h1, BYTE_LEN_256B);		
		debug_out("M3 E-Hash2", pSta->e_h2, BYTE_LEN_256B);
	}
#endif	
	pSta->tx_timeout = pCtx->tx_timeout;
...
static int msgHandler_M6(CTX_Tp pCtx, STA_CTX_Tp pSta, unsigned char *pMsg, int msg_len)
{
	...
	if (memcmp(tmp, pSta->r_h1, BYTE_LEN_256B)) {
		...
		debug_out("My keep:", pSta->r_h1, BYTE_LEN_256B);
	...
	if (memcmp(tmp, pSta->r_h2, BYTE_LEN_256B)) {
		...
		report_WPS_STATUS(HASH_FAIL);
#ifdef	AUTO_LOCK_DOWN
		if(pCtx->brute_force_attack_mitigation)
			record_and_check_AuthFail_v2(pCtx,pSta->addr);
		else
