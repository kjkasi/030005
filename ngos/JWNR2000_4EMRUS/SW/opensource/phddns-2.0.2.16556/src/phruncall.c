...
/* Netgear start */
		...
		{
			if(++i < argc){
				parameter->persist_cmd = malloc(strlen(argv[i]) + 1);
				if(parameter->persist_cmd) {
					strcpy(parameter->persist_cmd, argv[i]);
				}
			}
		}
/* Netgear end */
...
/* Netgear start */
	printf("  -v, set the loglevel to LOG_DEBUG\n");
	printf("  it is valid only when configuration file is used\n");
	printf("--persist\n");
	printf("  -p, external command to get or set the last Update Status");
/* Netgear end */
	printf("--user\n");
...
/* Netgear start */
	if (parameter->persist_cmd)
	{
		pglobal->persist_cmd = malloc(strlen(parameter->persist_cmd) + 1);
		if(pglobal->persist_cmd) {
			strcpy(pglobal->persist_cmd, parameter->persist_cmd);
		}
	}
/* Netgear End */
	return 0;
...
/* Netgear start */
	memset(buffer,0,sizeof(buffer));
	sprintf(buffer,"szpersist_cmd = %s \n",parameter->persist_cmd);
	fputs(buffer,fp);
/* Netgear End */
...
int MyReadFile(char* filename,PHGlobal *pglobal,PH_parameter *parameter)
{
	...
	while( memset(buffer,0,sizeof(buffer)) && fgets(buffer,sizeof(buffer),fp) && circle < 6 ) {
...
/* Netgear start */
		else if( !strcmp(attribute,"szpersist_cmd") ){
			parameter->persist_cmd = malloc(strlen(tvalue));
			if(parameter->persist_cmd) {
				strcpy(parameter->persist_cmd,tvalue);
			}
		}
/* Netgear End */
		circle ++;
